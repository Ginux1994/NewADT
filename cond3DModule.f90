...
contains
    ...
    ! read element inf
            ...
            if((analyType>0).and.(ind == 2)) then 
				...
				if(nSHnd>0) then	
					do i=1,nSHnd
						ndID = SHNdID(i)						
						if(CmatType==0) then
							C0mMat(ndID, ndID) = ndSH(i)
						else 
							C1Mat(ndID) = C1Mat(ndID) + ndSH(i)
						end if
					enddo
				end if
                if(nLatht>0) then
		...
		use heatFlowInf, only: heatGenInf, heatGenElmID, heatGenValue
		implicit none
		...
		do i=1,nElm
			...
			heatGen = heatGenValue(1, grpID)
            call cal3DHeatGenVec(elmID, 8, 0, coords, heatGen, res)
            call getNdIDByElmID(elmID, 8, ndID)
			call assemElmVec(res, ndID)
		enddo		
