...
!*****************************************************************************    
	...
	use timeStepInf
	use timeFuncInf
    real, allocatable:: fixTempNdInf(:,:), fixTempNdValue(:,:), convNdInf(:,:), convNdValue(:,:), radiaNdInf(:,:), radiaNdValue(:,:), load1Inf(:,:), load1Value(:,:)
contains
...
10							sloop = (timeFuncAt(pointLoop+1, curveID) - timeFuncAt(pointLoop, curveID))/&
        ...
        subroutine read_ConvNdFlow
            integer:: i, j, k, ndID, curveID, fac, startTime, kn
            allocate(convNdInf(4,nConvNd), convNdValue(stepNumAt(iper), nFixTempNd))
        ...
        subroutine read_RadiaNdFlow
            ...
            allocate(radiaNdInf(4,nRadiaNd), radiaNdValue(stepNumAt(iper), nFixTempNd))
        ...
        subroutine read_Load1
            ...
            allocate(load1Inf(4,nLoad1), load1Value(stepNumAt(iper), nFixTempNd))
    ...
    ! read element inf
        ...
        use nodeInf, only:dofID
            ...
            if (index==1) then
                ...
                enddo
            else if((analyType>0).and.(index == 2).or. (index==3))then 
...
if(index==1) then           
            if(haveLnConvOrFixTempLoad==1) then ! 如果有线性对流或固定温度，继续assem linear matrix
                ...
                index=2       
            else if((nSHnd>0).and.(nLatht.ne.0)) then ! 总装热容矩阵
